...
class MatchingModel(nn.Module):
    ...
    def run_attack_input(self, *args, **kwargs):
        return Runner.run_attack_input(self, *args, **kwargs)

    def run_attack_input2(self, *args, **kwargs):
        return Runner.run_attack_input2(self, *args, **kwargs)

    def add_candidate_tokens(self, *args, **kwargs):
        return Runner.add_candidate_tokens(self, *args, **kwargs)
    ...
    def get_input_tensor(self, *args, **kwargs):
        return Runner.get_input_tensor(self, *args, **kwargs)
    ...
    def model_get_input_tensor(self, input):
        embeddings = {}
        for name in self.meta.all_text_fields:
    ...
    def forward2(self, input, word_index, word_embedding):
        ...
        for name in self.meta.all_text_fields:
            attr_input = getattr(input, name)
            embeddings[name] = self.embed[name](attr_input)
            curr_num_word = embeddings[name][0][0].shape[0] - 2

            if count + curr_num_word > word_index:
                embeddings[name][0][0][word_index - count + 1] = torch.from_numpy(word_embedding)
                count = float('-inf')
            else:
                count = count + curr_num_word
        ...
        return self.classifier(entity_comparison), embeddings
    ...
    def forward(self, input):
        ...
        for name in self.meta.all_text_fields:
            attr_input = getattr(input, name)
            embeddings[name] = self.embed[name](attr_input)
            embeddings[name][0].requires_grad_(True)
            embeddings[name][0].retain_grad()
